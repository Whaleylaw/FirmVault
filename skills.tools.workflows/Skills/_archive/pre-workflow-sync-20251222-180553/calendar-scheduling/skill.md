...
| `render_ui_script` | Render calendar UI in the workbench (use for display) |
...
### Render in Workbench Calendar Pane (Preferred for UI)

When the user asks to *show* the calendar (today/tomorrow/a date range), call `render_ui_script`
so the calendar pane actually renders events.

```python
# Show tomorrow in the UI calendar pane
render_ui_script(
    script_path="UI/calendar/show_day.py",
    script_args=["--date", "2025-12-17", "--days", "1", "--calendar-id", "primary"]
)
```

You can still call `list_events(...)` for a textual summary, but `render_ui_script` is the
UI-native way to display events.
