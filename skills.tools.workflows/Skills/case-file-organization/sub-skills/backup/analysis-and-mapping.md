...
- ✅ **Use `read_multimodal_file` tool to read PDFs directly**
...
## Hash Markdown Files and De-Biased Content

**CRITICAL: Hash-style markdown filenames (e.g., `00cab744d0f246a8....md`) are intentionally random and carry NO semantic information.**

Desktop can read PDFs directly with `read_multimodal_file`. These hash-named files may exist from legacy preprocessing but are no longer the primary reading method.

**You MUST:**
- ✅ Treat each hash `.md` file as the **authoritative representation** of a source document (medical record, bill, pleading, etc.)
- ✅ **Open and read the markdown content** to determine:
  - Document type (medical record, police report, demand, discovery, lien, etc.)
  - Correct bucket (one of the 8 categories)
  - Correct date using the Dating Protocol
  - Proper Originator and Description for the filename
- ✅ Base ALL categorization and naming decisions **only on the document content**, NOT the hash filename

**You MUST NOT:**
- ❌ Infer that a hash `.md` file is a "note", "draft", or any specific type based on its filename
- ❌ Use the hash filename as a signal for bucket, originator, or description
- ❌ Create pseudo-buckets like "Working Notes / AI Drafts" based solely on hash names

> Any attempt at naming or categorizing that uses the hash filename as guidance is **objectively wrong** and violates this protocol.

Desktop can read PDFs directly - no conversion or mapping needed. Your job in Phase 2 is to correctly classify and name files based on their content, using `read_multimodal_file` for direct PDF access.
...
```bash
# Use read_multimodal_file to read PDFs directly
read_multimodal_file("/case/documents/document.pdf")
```
