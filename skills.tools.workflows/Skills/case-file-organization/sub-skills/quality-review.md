...
## ⚠️ REMINDER: Hash-Named Files Are Intentional

Files with 32-character hex names like `00cab744d0f246a885e0bd8ebb2960a7.md` are **NOT** system files, garbage, or corrupted files. They are **REAL case documents** from a legacy workflow that converted PDFs to markdown with random hash names to prevent filename bias. Desktop now reads PDFs directly with `read_multimodal_file` - no conversion needed.

**When re-evaluating these files:**
- Read the CONTENT inside each `.md` file
- Verify Phase 2 correctly identified the document type from content
- The hash filename tells you nothing - only the content matters
- Don't skip them thinking they're system files!
...
1. **Re-check Every File** - Read each `.md` document and verify Phase 2's categorization and naming
...
2. **Confirm they're actually duplicates:**
   - Same content (read both `.md` files to verify)
   - Different hash names but identical content
   - Same date + same type (court notices)
3. **Check companion files** - If email, ensure both .eml and .md are handled together
...
|---------------------|--------------|----------|
| doc_0099.md | doc_0001.md | ✅ Same content |
| email_notice-alt.md | email_notice.md | ✅ Same date/type |
```
...
**You ONLY read markdown (.md) files.** All documents have been converted to markdown.

```python
# Read any markdown file directly
read_file("projects/{case_name}/doc_0001.md")
read_file("projects/{case_name}/subfolder/doc_0042.md")
read_file("projects/{case_name}/email_something.md")
```

### For Each File in the Reorganization Map:

1. **Read the `.md` file:**
   - Use `read_file()` to open the markdown content
   - For emails, read the `.md` companion
...
Create review folder if needed using FilesystemBackend tools.
...
|--------------|----------------------|-------------|----------------|
| 00cab744d0f246a885e0bd8ebb2960a7.md | 2024-03-15 - Client - Medical Record - Jewish - Visit | Wrong facility (should be UofL) | Rename with correct facility |
| a7745a1e94294334af0412d9ccc11dd6.md | 2024-01-01 - Client - Investigation - Incident Report | Wrong category (actually medical record) | Move to Medical Records/ |
| b904f81746e940c39366cca164970b02.md | 2024-05-01 - Abby - Litigation - Plaintiff - Nayram Answers | Wrong client folder | Move to Nayram's folder |
```
...
**Location:** `projects/{case_name}/REVIEW_NEEDED_Phase_3/`
**Total Files:** {count}
...
**Error Rate ≤ 20%:**
1. Main agent reviews `projects/{case_name}/REVIEW_NEEDED_Phase_3/` folder
2. Main agent makes final decision on flagged files
3. If approved, proceed to Phase 4 execution

**Error Rate > 20%:**
1. User reviews this summary
2. User examines flagged files in `projects/{case_name}/REVIEW_NEEDED_Phase_3/`
...
- Two hash-named files with identical content → DELETE one
- E.g., `00cab744...md` and `b904f817...md` have same content → Keep one
...
- [ ] Read all `.md` files to verify content matches proposed names
