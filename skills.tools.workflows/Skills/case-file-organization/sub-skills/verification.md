...
3. **Verify companion .md files** have matching names (for emails and any remaining PDFs)
...
Check that all 8 bucket directories exist using FilesystemBackend:
...
**Expected:** Only directories should be listed (no loose files)
...
ls("projects/{case_name}/Case Information/")
...
For each bucket, verify files that should have .md companions:
1. List all files in the bucket
2. Check if `.md` and corresponding `.pdf` or `.eml` files have matching base names

**What to look for:**
- `2024-03-15 - Client - Medical Record - Provider - Description.pdf` should have
- `2024-03-15 - Client - Medical Record - Provider - Description.md`
...
Generate file counts for each directory using FilesystemBackend:

```python
# List each bucket and count files
ls("projects/{case_name}/Case Information/")
ls("projects/{case_name}/Client/")
ls("projects/{case_name}/Investigation/")
ls("projects/{case_name}/Medical Records/")
...
**Note:** Count includes both main files (.pdf, .eml) and .md companions
...
- Check log for failed operations
...
- `filename1.md` - Reason: Flagged for review
- `filename2.md` - Reason: Not in mapping

### Naming Convention Issues

- `wrong-format.md` (in Medical Records/) - Issue: Date format is MM-DD-YYYY instead of YYYY-MM-DD
- `missing-client.md` (in Client/) - Issue: Client name missing from filename
...
- Note files that need markdown generation
...
- Note missing .md files for future processing
...
- ⚠️ 1-2 files missing .md companions (can be noted for future)
