...
- Track request in `cases/<slug>/contacts/` (provider stubs) and `## Medical Providers` section
...
2. IDENTIFY PROVIDER
   └── Get provider name from `cases/<slug>/contacts/` (provider stubs) and `## Medical Providers` section
...
8. TRACK & FOLLOW-UP
   └── Record sent date in `cases/<slug>/contacts/` (provider stubs) and `## Medical Providers` section
   └── Schedule 14-day follow-up
```
...
provider_name = "UK Hospital"  # From `cases/<slug>/contacts/` (provider stubs) and `## Medical Providers` section
...
- Tracking updated in `cases/<slug>/contacts/` (provider stubs) and `## Medical Providers` section
