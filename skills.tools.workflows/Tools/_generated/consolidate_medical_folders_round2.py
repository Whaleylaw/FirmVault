#!/usr/bin/env python3
"""
Round 2: Consolidate remaining duplicate medical provider folders
"""
...
# Round 2 consolidation mapping
CONSOLIDATION_MAP = {
    "Baptist Health Medical Group Orthopedics Floyd": [
        "Baptist Health Medical Group Orthopedics - Floyd"
    ],
    "Medical Aime Accidbance Treatment": [
        "Medical Aime Accidbance"
    ]
}
...
def consolidate_folders(case_path, dry_run=False):
    ...
    print(f"{'🔍 DRY RUN MODE' if dry_run else '🚀 EXECUTING'}: Medical Folder Consolidation (Round 2)")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python consolidate_medical_folders_round2.py <case_folder_name> [--dry-run]")
        sys.exit(1)
