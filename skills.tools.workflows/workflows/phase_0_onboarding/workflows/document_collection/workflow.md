...
> All case data is stored in FalkorDB and accessed via `query_case_graph()` or graph manager functions.
> See [Roscoe Knowledge Graph Schema.md](/src/roscoe/agents/paralegal/# Roscoe Knowledge Graph Schema.md) for entity types.
...
For complete document requirements by case type, see **[../../landmarks.md](../../landmarks.md#additional-documents-not-landmarks)**.

This workflow collects all required documents identified in the landmarks. The checklist is dynamically generated based on:
- Case type (MVA, S&F, WC)
- Employment status (for wage verification)
- Medicare status (for CMS verification)
...
Update graph entity using write_entity() with DocuSign status:
