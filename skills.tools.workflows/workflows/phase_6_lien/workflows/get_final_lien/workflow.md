...
**Query:**
```python
# Use graph query to get outstanding liens
execute_python_script("/Tools/queries/get_case_liens.py", ["Case-Name", "--status", "outstanding"])
```
...
**Update using graph write:**
```python
# Create or update Lien entity in knowledge graph
write_entity(
    entity_type="Lien",
    properties={
        "name": "Medicare Lien",
    ...
    relationships=[
        {"type": "LIEN_ON_CASE", "target_entity": "Case-Name"}
    ]
)
```

**Note:** See `/workspace/KNOWLEDGE_GRAPH_SCHEMA.md` for complete Lien entity schema.
