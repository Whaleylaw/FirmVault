...
**Auto-fill fields** (handled by tool from graph queries):
- `{{client.name}}` - From graph query via `get_case_overview.py`
- `{{defendant.name}}` - From graph query for liable party entities
- `{{incidentDate}}` - From graph query via `get_case_overview.py`
- `{{firm.attorney}}`, `{{firm.barNumber}}`, etc. - From auto-loaded context or firm config

**Note:** Case context is automatically injected by CaseContextMiddleware when client name is mentioned.
