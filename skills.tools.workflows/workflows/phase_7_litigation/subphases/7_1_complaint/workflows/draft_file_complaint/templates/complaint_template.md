...
| Field | Type | Source |
|-------|------|--------|
| `{{client.name}}` | Auto | Graph query via `get_case_overview.py` |
| `{{defendant.name}}` | Auto | Graph query for liable party entities |
| `{{incidentDate}}` | Auto | Graph query via `get_case_overview.py` |
| `{{firm.attorney}}` | Auto | Firm config or auto-loaded context |
| `{{firm.barNumber}}` | Auto | Firm config or auto-loaded context |
| `{{firm.name}}` | Auto | Firm config or auto-loaded context |
| `{{firm.address}}` | Auto | Firm config or auto-loaded context |
| `{{firm.phone}}` | Auto | Firm config or auto-loaded context |
| `[COUNTY]` | Agent | Case venue decision |
| `[FACTS]` | Agent | Agent writes based on case |
| `[INJURIES]` | Agent | Medical chronology from graph |
| `[NEGLIGENT ACTS]` | Agent | Agent analyzes |

**Note:** Auto-fill fields are populated from knowledge graph queries or auto-loaded case context (CaseContextMiddleware).
