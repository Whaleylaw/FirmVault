...
**Updates:**
```python
# Update case closure status in knowledge graph
write_entity(
    entity_type="Case",
    properties={
        "name": "Case-Name",
...
**Updates:**
```python
# Update review request status in knowledge graph
write_entity(
    entity_type="Case",
    properties={
        "name": "Case-Name",
        "closure_review_requested": true,
        "closure_review_requested_date": "{{today}}"
    },
    relationships=[]
)
```
...
**Updates:**
```python
# Update case archive status in knowledge graph
write_entity(
    entity_type="Case",
    properties={
        "name": "Case-Name",
        "closure_archived_date": "{{today}}",
        "closure_archive_location": "{{location}}",
        "closure_retention_until": "{{today + 5 years}}",
        "status": "closed",
...
On completion, update case entity in knowledge graph:
```python
# Update case closure state
write_entity(
    entity_type="Case",
    properties={
        "name": "Case-Name",
        "current_phase": "closed",
        "status": "closed",
        "closure_completed_date": "{{today}}",
        "closure_final_letter_sent": true,
        "closure_review_requested": "{{true/false}}",
        "closure_archived": true,
        "closure_archive_location": "{{location}}",
        "closure_retention_until": "{{date}}"
    },
    relationships=[]
)
```

**Note:** See `/workspace/KNOWLEDGE_GRAPH_SCHEMA.md` for complete Case entity schema.
